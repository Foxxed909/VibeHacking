...
| `mcp_probe` | **MCP (Model Context Protocol) server auditor.** Detects the JSON-RPC endpoint/transport, then tests the auth boundary **with no session on purpose** — a valid `initialize`/`tools/list` result unauthenticated is the finding. Enumerates exposed tools/resources/prompts and flags dangerous ones (shell/file/fetch/db/secret), and checks CORS for the any-origin-with-credentials bug (DNS-rebinding-class). `--authed` re-runs with your captured session to show your account's tool surface. `--url`. |
