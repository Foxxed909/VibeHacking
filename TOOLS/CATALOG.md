...
| `takeover` | **Subdomain takeover detection.** Resolves the CNAME chain over DNS-over-HTTPS, matches known takeover-prone services (S3/GH Pages/Heroku/Azure/Fastly/Shopify/Netlify/…), and calls a takeover only when the CNAME target is NXDOMAIN *or* the live response carries the service's unclaimed-resource fingerprint. Pointing at a live service is reported safe. `--host`/`--list`. |
